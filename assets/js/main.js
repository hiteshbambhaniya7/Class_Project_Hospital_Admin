(function () {
  ...
  function donutMorrisjs() {
    if ($("#donutMorrisjs").length) {
      Morris.Donut({
        ...
        colors: ["#336cfb", "#64B5F6", "#e9e165", "#ed5564"],
      });
    }
  }
  ...
  function initLeafletMap() {
    if ($("#leaflet-map").length) {
      const mapboxAccessToken =
        "pk.eyJ1IjoidHlwZTBjb2RlIiwiYSI6ImNrdmNxejgxcjJsYTQydG9rbjdxZzBrdGoifQ.L35XjKdGNpJDk7cGS6UIaA";
  ...
  function settings() {
    ...
    if (modal.length) {
      if (
        !getCookie("MAClosedSettings") ||
        getCookie("MAClosedSettings") === "false"
      ) {
        setTimeout(() => {
          modal.modal("show");
        }, 500);
